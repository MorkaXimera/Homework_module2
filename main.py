...
def use_app():
	...
	if menu_number == '1':
		...
		user_transactions = csv_reader(csv_file_path="C:/Users/morka/PycharmProjects/files_for_homework_2/transactions.csv")
	else:
		user_transactions = xlsx_reader (excel_file_path="C:/Users/morka/PycharmProjects/files_for_homework_2/transactions_excel.xlsx")
	...
	if len(final_transactions_list) > 0:
		...
		for transaction in final_transactions_list:
			...
			if transaction['description'] == 'Перевод с карты на карту':
				...
				print(f'{card_out_name} {get_mask_card_number(card_out_number)} -> {card_in_name} {get_mask_card_number(card_in_number)}')
			if transaction['description'] == 'Перевод организации':
				...
				if card_out_name == 'Счет ':
					...
					print(f'{card_out_name} {get_mask_card_number(card_out_number)} -> Счет {get_mask_account(account_in)}')
			if transaction['description'] == 'Перевод со счета на счет':
				print(f'Счет {get_mask_account(transaction['from'][5:])} -> Счет {get_mask_account(transaction['to'][5:])}')
	else:
